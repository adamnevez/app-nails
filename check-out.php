...
<html lang="pt-br">
    <head>
        ...
        <title>Webstore - formulario de chekout</title>
    </head>
    <body>        
        ...
        if (isset($data['BtnPicPay'])) {
            ...
            if (!$empty_input) {
                ...
                $query_pay_picpay = "INSERT INTO payments_picpays 
                (first_name, last_name, cpf, phone, email, expires_at, product_id, created) 
                VALUES (:first_name, :last_name, :cpf, :phone, :email, :expires_at, :product_id, :created)";
                ...
                if ($add_pay_picpay->rowCount()) {
                    ...
                    if (isset($data_result->code) AND $data_result->code != 200) {
                        ...
                        $query_up_pay_picpay = "UPDATE payments_picpays 
                        SET payment_url = '".$data_result->paymentUrl."',
                        qrcode = '".$data_result->qrcode->base64."', 
                        modified = NOW() 
                    ...
                    ?>">
                        ...
                        <div class="form-group">
                            <label for="email">Agendamento</label>
                                <select class="form-control col-6" name="dataagenamento" id="dataagenamento" value="<? if (isset($data['dataagenamento'])) { echo $data['dataagenamento']; } ?>">
                                    <option value="">Selecione Agendamento</option>
                                    <option value=""><?=date('d-m-Y')?></option>
                                    <option value=""><?=date('d-m-Y')?></option>
                                    <option value=""><?=date('d-m-Y')?></option>
                                    <option value=""><?=date('d-m-Y')?></option>
                                </select>
                            <input type="email" name="email" id="email" class="form-control" placeholder="Digite o seu melhor e-mail" value="<? if (isset($data['email'])) { echo $data['email']; } ?>">
                        </div> 
                        -->
